...
int main(int argc, char *argv[]) 
{
	...
	int totalBlocks = (N+(threadsPerBlock-1))/threadsPerBlock;
  	...
  	thrust::host_vector<double> h_x_serie1(N, 0);
 	thrust::device_vector<double> d_x_in_serie1 = h_x_serie1;
  	reloj.tic();
    thrust::transform(thrust::make_counting_iterator(1),thrust::make_counting_iterator(N),d_x_in_serie1.begin(),mi_operacion_inclusive());
  	thrust::inclusive_scan(d_x_in_serie1.begin(),d_x_in_serie1.end(),d_x_in_serie1.begin());
  	//thrust::inclusive_scan(d_x_in_serie1.begin(),d_x_in_serie1.end(),d_x_in_serie1.begin(), thrust::divides<float>()/*mi_operacion_inclusive_1()*/);
